...
class DownloadsTableModel extends AbstractTableModel implements Observer {
	...
	private static final long serialVersionUID = 8911848811987677451L;

	// These are the names for the table's columns.
	private static final String[] columnNames = { "URL", "Size", "Progress",
			"Status", "Duration", "End of download" };
	...
	public Object getValueAt(int row, int col) {
		...
		case 4: // Duration
			if (download.getStatus() == Download.COMPLETE) {
				long duration = download.getEnd() - download.getStart();
				// if (duration >= 1000) {
				// return String.valueOf(duration / 1000) + "." + ((duration /
				// 1000) - 1) * 1000 + " s";
				// } else {
		...
		case 5: // End of download
			if (download.getStatus() == Download.COMPLETE) {
				DateFormat formatter = DateFormat.getDateTimeInstance(
						DateFormat.MEDIUM, DateFormat.MEDIUM);
				Date date = new Date(download.getEnd());
